...
import cn.bugstack.middleware.dynamic.thread.pool.sdk.domain.impl.DynamicThreadPoolService;
...
import cn.bugstack.middleware.dynamic.thread.pool.sdk.trigger.job.ThreadPoolReportJob;
...
public class DynamicThreadPoolAutoConfig {
    ...
    public DynamicThreadPoolService dynamicThreadPoolService(ApplicationContext applicationContext, Map<String, ThreadPoolExecutor> threadPoolExecutorMap, RedissonClient redissonClient) {
        ...
        for (String key : keySet) {
            ThreadPoolConfigEntity threadPoolConfigEntity = redissonClient.<ThreadPoolConfigEntity>getBucket(RegistryEnumVO.THREAD_POOL_CONFIG_PARAMETER_LIST_KEY.getKey().concat("_").concat(applicationName).concat("_").concat(key)).get();
    ...
    public RTopic rTopic(RedissonClient redissonClient, ThreadPoolConfigAdjustListener threadPoolConfigAdjustListener) {
        RTopic topic = redissonClient.getTopic(RegistryEnumVO.DYNAMIC_THREAD_POOL_REDIS_TOPIC.getKey().concat("_").concat(applicationName));
